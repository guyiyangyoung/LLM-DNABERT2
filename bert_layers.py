...
class RoPEEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=512):
        ...
        self.register_buffer("sin", freqs.sin().repeat_interleave(2, dim=-1))
        self.register_buffer("cos", freqs.cos().repeat_interleave(2, dim=-1))
...
class BertEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.num_attention_heads = config.num_attention_heads
        self.rope = RoPEEmbedding(config.hidden_size, max_position_embeddings=config.max_position_embeddings)
    ...
    def forward(
        ...
        if subset_mask is None:
            for layer_module in self.layer:
                # Apply RoPE to query and key before passing into self-attention
            ...
            hidden_states_q, hidden_states_k = self.rope(hidden_states, hidden_states)
            hidden_states = self.layer[-1](hidden_states_q, hidden_states_k, cu_seqlens, seqlen, subset_idx=subset_idx, indices=indices, attn_mask=attention_mask, bias=extended_attention_mask)
